...
flowchart TD
    ...
    %% 시작 지점
    %% =========================
    A[시스템 시작] --> B[변수 초기화<br>ADC/PWM/EXTI/LCD 초기 설정]
    B --> C[ADC 반복 측정 시작]
    C --> D[현재 숫자 adc_val 표시 (LCD)]
    ...
    D --> E{TACT2 버튼 입력?}
    E -->|아니오| C
    E -->|예| F[security_mode = SUPERVISOR_INPUT]

    %% =========================
    %% SUPERVISOR PW 입력
    %% =========================
    F --> G[관리자 PW 입력 (다이얼+TACT1)]
    G --> H{PW 일치?}

    H -->|NO| A   %% 초기화 후 다시 시작
    H -->|YES| I[security_mode = CURRENT_INPUT]
    ...
    J --> K{PW 일치?}
    K -->|NO| A
    K -->|YES| L[security_mode = NEW_PASSWORD_INPUT]

    %% =========================
    %% 새 비밀번호 설정
    %% =========================
    L --> M[새 비밀번호 입력]
    M --> N[password[]에 저장<br>set_password()]
    N --> O[security_mode = IDLE]
    ...
    P -->|YES| Q[PWM = 2500us<br>금고 해제]
    P -->|NO| R[PWM = 1500us<br>금고 잠금]
    ...
    %% EXTI 디바운싱 처리
    %% =========================
    subgraph EXTI Interrupt
        S[버튼 인터럽트 발생] --> T{200ms 경과?}
        T -->|NO| U[입력 무시]
        T -->|YES| V[유효 입력으로 처리]
    end
```
